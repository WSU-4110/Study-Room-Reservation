/**
 * NotificationHelper - Handles notification logic for study room reservations
...
export type NotificationType = "reminder" | "confirmation" | "cancellation" | "upcoming";
...
export class NotificationHelper {
	...
	shouldSendReminder(
		...
		return reminderMinutes.some(
			(threshold) => minutesUntil <= threshold && minutesUntil > threshold - 1,
		);
	}
		...
		if (!Array.isArray(settings.reminderMinutes)) {
			...
			settings.reminderMinutes.forEach((min, index) => {
				if (typeof min !== "number" || min < 0) {
					errors.push(`reminderMinutes[${index}] must be a non-negative number`);
				}
			});
		}

		if (typeof settings.urgentThreshold !== "number" || settings.urgentThreshold < 0) {
	...
	groupNotificationsByType(
		notifications: Array<{ notification: ReservationNotification; type: NotificationType }>,
